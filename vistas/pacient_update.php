...
function generarCodigo() {
    ...
    fetch('./php/obtenerUltimoCodigo.php')
        ...
        .then(data => {
            let consecutivo = "01"; // Valor inicial del consecutivo
            if (data && data.ultimoCodigo) {
                const ultimoCodigo = data.ultimoCodigo;

                // Verificar si el código extraído sigue el formato esperado
                const regex = /^[DLWJVS][A-Z]{3}\d{2}D\d{2}$/; // Ejemplo: LENE01D01
                if (regex.test(ultimoCodigo)) {
                    // Extraer el consecutivo del último código (últimos 2 caracteres)
                    const consecutivoExtraido = parseInt(ultimoCodigo.slice(-2), 10);
                    if (!isNaN(consecutivoExtraido)) {
                        consecutivo = ("0" + (consecutivoExtraido + 1)).slice(-2); // Incrementar y asegurar 2 dígitos
                    }
                }
            }
